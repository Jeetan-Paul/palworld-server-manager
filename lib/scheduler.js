...
async function tick() {
  ...
  for (const s of dbm.listSchedules()) {
    ...
    try {
      logger.info("scheduler", `running due ${s.job_type} job for world ${s.world_id}`);
      ...
      logger.error("scheduler", `${s.job_type} job for world ${s.world_id} failed: ${e.message}`);
    }
  }
}
...
async function updateWorld(worldId, onLog = () => {}, jobId = null) {
  ...
  try {
    ...
    logger.info("update", `"${w.display_name}" updated to build ${bid || "?"}`);
    ...
    dbm.updateWorld(worldId, { status: "stopped" });
    logger.error("update", `update of "${w ? w.display_name : worldId}" failed: ${e.message}`);
