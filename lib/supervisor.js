...
async function startWorld(worldId) {
  ...
  logger.info("supervisor", `world "${world.display_name}" spawned ${path.basename(bin)} (pid ${child.pid})`);
  ...
  child.on("close", (code) => {
    ...
    if (w && w.status !== "stopping" && w.status !== "updating") {
      ...
      logger.warn("supervisor", `world "${w.display_name}" exited unexpectedly (code ${code}) — will be flagged for the guardian`);
      ...
      logger.info("supervisor", `world "${w ? w.display_name : worldId}" stopped (code ${code})`);
    }
  });
...
async function guardTick() {
  ...
  for (const w of worlds) {
    ...
    if (w.status === "crashed") {
      ...
      logger.warn("guardian", `world "${w.display_name}" crashed — auto-restarting (crash #${(w.crash_count || 0) + 1})`);
      try { await startWorld(w.world_id); }
      catch (e) { logger.error("guardian", `auto-restart of "${w.display_name}" failed: ${e.message}`); }
      continue;
    }
    if (w.status === "running") {
      ...
      if (w.rest_api_enabled) {
        ...
        if (!ok) {
          ...
          if (n >= 6) { // ~2 min unresponsive
            ...
            logger.warn("guardian", `world "${w.display_name}" REST API unresponsive ~2min — force restarting (hang)`);
            try { await restartWorld(w.world_id); }
            catch (e) { logger.error("guardian", `force-restart of "${w.display_name}" failed: ${e.message}`); }
          }
        } else {
