...
async function provisionWorld(jobId, worldId) {
  ...
  logger.info("provision", `installing "${world.display_name}" → ${world.install_dir}`);
  try {
    ...
    logger.info("provision", `"${world.display_name}" install complete (build ${bid || "?"})`);
    log("Done. World is ready to start.");
    jobs.finishJob(jobId, true, { worldId });
  } catch (e) {
    log(`ERROR: ${e.message}`);
    logger.error("provision", `install of "${world.display_name}" failed: ${e.message}`);
    dbm.updateWorld(worldId, { status: "stopped" });
